...
默认评测以 `V5` 中的 `zhangsan` 为测试人格：信息技术部、`INTERNAL` 密级。因此 VPN 和公开的考勤问题应当回答；财务、采购、销售、客服等其他部门的内部文档，以及机密或严格受限文档应当拒答。这使 20 条用例同时验证“找得到”和“不该看到”。
...
页面包含文档导入、Prompt 创建/启用/回滚、检索与答案评测、运行历史、逐题详情和两次运行对比。页面资源可以公开加载，但不会公开任何业务数据；所有 API 请求仍必须携带 JWT。

本地联调时可以生成一个同时包含三个管理权限的令牌：

```bash
ADMIN_TOKEN="$(./scripts/generate-dev-jwt.sh \
  zhangsan \
  00000000-0000-0000-0000-000000000001 \
  'knowledge.write prompt.manage evaluation.run')"
```

在管理页面粘贴该令牌即可连接。令牌只保存在当前浏览器标签页的 `sessionStorage`，关闭标签页后会被清除，不会写入数据库或项目文件。答案评测按钮会在真实调用模型前再次要求确认。
